...
CREATE TABLE    Master_Location_Municipality(
  ...
  CREATE TABLE    Master_Location_Departament(
   ...
   alter table detail_bet add quota_bookmaker number(10,0) null;


   CREATE TABLE "LEGITTRIP"."WITHDRAW_REJECTED" (
    ID NUMBER(10,0) GENERATED ALWAYS AS IDENTITY MINVALUE 1 MAXVALUE 999999999 , 
    FK_ID_WITHDRAW NUMBER(30,0), 
    DATE_REJECTED TIMESTAMP , 

    Flag VARCHAR2(15),
    CONSTRAINT WITHDRAW_REJECTED PRIMARY KEY (Id))
    TABLESPACE BET_ITM
    STORAGE ( INITIAL 50K);

    CREATE TABLE "LEGITTRIP"."DEPOSITE_REJECTED" (
    ID NUMBER(10,0) GENERATED ALWAYS AS IDENTITY MINVALUE 1 MAXVALUE 999999999 , 
    FK_ID_DEPOSITE NUMBER(30,0), 
    DATE_REJECTED TIMESTAMP , 

          Flag  VARCHAR2(15),
       CONSTRAINT DEPOSITE_REJECTED PRIMARY KEY (Id))
     TABLESPACE BET_ITM
     STORAGE ( INITIAL 50K);
